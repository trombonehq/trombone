...
export function addMethods() {
  Meteor.methods({
    ...
    getActiveUsers(range) {
      const user = Meteor.user();
      if (!user || !Roles.userIsInRole(user, ['super-admin'])) {
        throw new Meteor.Error(401, 'You need to be an admin to get active user counts.');
      }
      const superUserId = user._id;

      const rangeLookup = {
        week: (3600000 * 24) * 7,
        thirtydays: (3600000 * 24) * 30,
        hour: 3600000,
      };

      const rangeFromToday = new Date(Date.now() - rangeLookup[range]);

      return Meteor.users.find({ $and: [
        { _id:
          { $ne: superUserId },
        },
        { lastLogin: { $gt: rangeFromToday } },
      ] }).count();
