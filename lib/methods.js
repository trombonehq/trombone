addMethods = function() {
  Meteor.methods({
    getUsers: function(searchTerm, usersToLoad) {
      ...
      var selector = searchTerm ? searchTermSelector(searchTerm) : { _id: { $ne: superUserId } } ;

      return Meteor.users.find(selector, options).fetch();

    },

    getLastWeekSignUps: function() {
      var oneWeekAgo = new Date(Date.now() - ((((1000 * 60) * 60) * 24) * 7));
      return Meteor.users.find({'createdAt': { $gt: oneWeekAgo}}).count();
    },
